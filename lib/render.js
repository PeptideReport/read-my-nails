...
async function loadEmoji(code, segment) {
  ...
  const cps = Array.from(segment).map(c => c.codePointAt(0)).filter(cp => cp !== 0xfe0f).map(cp => cp.toString(16));
  const key = cps.join('_');
  if (emojiCache.has(key)) return emojiCache.get(key);
  const urls = [
    `https://raw.githubusercontent.com/googlefonts/noto-emoji/main/svg/emoji_u${key}.svg`,
    `https://cdn.jsdelivr.net/gh/googlefonts/noto-emoji@main/svg/emoji_u${key}.svg`,
  ];
  let data = '';
  for (const u of urls) { try { const r = await fetch(u); if (r.ok) { data = 'data:image/svg+xml;base64,' + Buffer.from(await r.text()).toString('base64'); break; } } catch (e) {} }
