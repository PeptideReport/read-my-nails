...
export async function grants(db, email, kind = 'chapter') {
  // Filtered in JS, not SQL. Production's credit_grants has no `kind` column until the schema.sql migration
// runs — an .eq('kind', kind) SQL filter against a column Postgres doesn't have yet returns a query error
// (not zero rows), and every call site here does `const { data } = await ...` without checking `error`, so
// that error would silently read as "no grants" for EVERY account and kind — including the existing
// library-chapter credits real customers already paid for, not just the new ai_gen overage. Selecting
// everything and filtering by kind afterward means a row with no `kind` field yet (pre-migration) is
// treated as 'chapter' — the column's own DB default — so chapter-credit balances keep working through
// the gap; 'ai_gen' correctly finds nothing until the migration lands and real ai_gen grants exist.
const { data } = await db.from('credit_grants').select('*').eq('email', norm(email)).eq('voided', false).gt('remaining', 0).order('expires_at', { ascending: true, nullsFirst: false });
  return (data || []).filter(g => (g.kind || 'chapter') === kind).filter(g => !g.expires_at || new Date(g.expires_at) > new Date());
}
