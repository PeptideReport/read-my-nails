...
if (out && typeof out.ok === 'boolean') { verdict = { ok: out.ok, flags: Array.isArray(out.flags) ? out.flags.slice(0, 5) : [] }; modelAnswered = true; }
} catch (e) { /* model down → fail open for THIS request only; never cache the guess */ }
// Only cache a real model verdict. Caching the fail-open default would mark a phrase safe forever after one hiccup.
if (modelAnswered) { try { await db.from('screen_cache').upsert({ mark_key: key, verdict }); } catch (e) {} }
return { ...verdict, source: modelAnswered ? 'model' : 'local' };
}
