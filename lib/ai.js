...
let _client;
// timeout keeps a slow call from hanging past the function's 60s budget; one retry rides out a transient blip.
const client = () => (_client ||= new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY, timeout: 50000, maxRetries: 1 }));
...
if (e?.status === 400 && /temperature/i.test(String(e?.message))) { delete req.temperature; r = await client().messages.create(req); }
else { console.error('[ai.ask] Anthropic call failed:', e?.status, e?.message || e); throw e; }
}
const text = r.content.filter(b => b.type === 'text').map(b => b.text).join('');
if (!json) return text;
const m = text.match(/\{[\s\S]*\}|\[[\s\S]*\]/);
if (!m) { console.error('[ai.ask] no JSON in model reply. stop_reason:', r.stop_reason, '| head:', text.slice(0, 300)); throw new Error('no json'); }
try { return JSON.parse(m[0]); }
catch (e) { console.error('[ai.ask] JSON.parse failed (reply likely truncated — raise maxTokens). stop_reason:', r.stop_reason, '| head:', m[0].slice(0, 200)); throw new Error('bad json'); }
}
...
export const HOUSE = `You write Read My Nails sets. A set is five fingernails, thumb to pinky, that read as one phrase, joke, pun or affirmation when the hand is held up. Each nail carries one MARK: a single letter, a short word, a number, or one emoji used as a word (🐝 = "be", 👁️ = "I/eye", 🍯 = "honey", 2 = "to/too"). Rebus is the house sport: the reader should have to sound it out for a second, then smile.
LEGIBILITY IS THE FIRST RULE. A mark is printed on one small fingernail, so it must be SHORT: never more than 5 characters, and 2–4 is better. Never put a whole long word on one nail — that is the single most common mistake. If a word is longer than 5 letters, split it across two nails at a natural break so it still reads left to right: SEASON → "SEA"+"SON", BESTIE → "BES"+"TIE", VILLAIN → "VILL"+"AIN", ENERGY → "EN"+"ERGY", GRANDMA → "GRAND"+"MA". Prefer a clean rebus shortening when one exists: FOREVER → "4EVR", TONIGHT → "2NITE", STRAIGHT → "STR8", GREAT → "GR8", GRADUATE → "GRAD"+"U8". A set is ALWAYS exactly five nails, so when a split needs an extra nail, drop a decorative-only emoji to make room — but NEVER drop an emoji that is carrying a sound (🐝 = be, 👁️ = I, 🌊 = sea, → arrow) and never drop a real word. Before you finish each set, count it: exactly five marks, and not one of them longer than five characters.
...
// Safety net so no printed nail can ever exceed 5 characters. The HOUSE prompt asks the model to
// split long words across nails; if one still slips through, shorten it in place (drop interior
// vowels, then hard-cap) so it prints legibly on the nail instead of bleeding off the edge.
function squeezeMark(m) {
if (m.length <= 5) return m;
// Array.from keeps emoji/accented letters whole, so the hard cap can never split a character in half.
const chars = Array.from(m);
for (let i = chars.length - 2; i > 0 && chars.join('').length > 5; i--) {
if ('AEIOUÁÉÍÓÚ'.includes(chars[i])) chars.splice(i, 1);
}
while (chars.length && chars.join('').length > 5) chars.pop();
return chars.join('');
}
...
const fix = hand => Array.isArray(hand) && hand.length === 5 ? hand.map(x => { const [m, p = ''] = String(x).split('|'); const mark = isEmoji(m.trim()) ? m.trim() : squeezeMark(m.trim().toUpperCase()); /* hard 5-char nail cap — see squeezeMark */ return mark + '|' + (POLISHES.includes(p.trim()) ? p.trim() : ''); }) : null;
if (!s || !s.a) continue;
...
const L = lang.toUpperCase();
const n = Math.min(24, Math.max(4, count));
// Budget the reply so a full chapter's JSON never truncates mid-set (truncation was throwing "the writer stumbled").
const maxTokens = Math.min(8000, 1600 + n * 380);
const out = await ask({
system: HOUSE + `\nLanguage for this chapter: ${LANG_NAME[L] || L}.`,
user: `Write a chapter of ${n} sets.\nTheme: ${theme}\n${audience ? 'Audience: ' + audience + '\n' : ''}${notes ? 'Notes from the salon: ' + notes + '\n' : ''}
...
maxTokens, json: true, temperature: 0.8,
});
