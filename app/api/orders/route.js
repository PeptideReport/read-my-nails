...
// This endpoint takes no PIN (walk-in customers place their own orders), so guard it two ways:
// 1) Rate limit per salon — a bad actor who knows the kiosk URL can't flood one queue. A real party rush
//    (a dozen friends tapping at once) stays well under this; sustained spam gets a 429.
const { count: recent } = await db.from('orders').select('id', { count: 'exact', head: true }).eq('tenant', tenant).gte('created_at', new Date(Date.now() - 60000).toISOString());
...
// 2) Cap the order payload — the kiosk sends the hands blob; reject an oversized one so nobody parks megabytes in the queue.
if (o.hands !== undefined && JSON.stringify(o.hands).length > 20000) return NextResponse.json({ error: 'That order is too large.' }, { status: 400 });
let row;
if (o.web) {
