...
export async function POST(req) {
  ...
  if (ev.type === 'checkout.session.completed') {
    ...
    if (s.mode === 'subscription') {
      ...
      // One-time purchases: Start pack, Whole Library, extra chapters.
      ...
      if (email) {
        ...
        if (p.credits) await grantCredits(db, { email, amount: p.credits, reason: key, ref: s.id });
  ...
  if (ev.type === 'invoice.paid') {
    ...
    if (subId) {
      ...
      if (p?.credits && email) await grantCredits(db, { email, amount: p.credits, reason: key, ref: inv.id, subscriptionId: subId });
