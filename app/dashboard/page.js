...
<h2 style={{ marginTop: 12 }}>Admin · creator royalties (30 days)</h2>
{me.royalties?.length ? <table className="ptable"><thead><tr><th>Chapter</th><th>Creator</th><th>Orders</th></tr></thead><tbody>{me.royalties.map(r => <tr key={r.code}><td><span className="mono">{r.code}</span> {r.title}</td><td>{r.creator?.name} {r.creator?.instagram ? '@' + r.creator.instagram : ''} <small className="muted">{r.creator?.email}</small></td><td>{r.orders}</td></tr>)}</tbody></table> : <p className="muted">No creator chapters live yet. Submissions arrive in “Write a chapter” above.</p>}
