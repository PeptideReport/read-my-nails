...
export default function sitemap() {
  ...
  const now = new Date();
  const fixed = ['/', '/es', '/library', '/salons', '/nail-printers', '/guides', '/creators', '/partners', '/own-a-location', '/es/own-a-location', '/free', '/terms', '/privacy'].map(p => ({ url: site + p, lastModified: now, changeFrequency: 'weekly', priority: p === '/' ? 1 : 0.8 }));
