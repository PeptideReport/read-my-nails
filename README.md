...
5. **Storage → New bucket** `library`, **Private**. Upload, with these exact names: `tiles-part1of3.zip`, `tiles-part2of3.zip`, `tiles-part3of3.zip` (the whole library — 2,265 tiles in `tiles/` plus sets.csv, tiles.csv and README.txt — split in three so each zip stays under Supabase's 50 MB per-file limit; the dashboard lists all three), `Read-My-Nails-Library.pdf` (the quick-reference book — regenerate any time with `node scripts/reference-pdf.js`, it reads `data/library.json`), `menu-cards.zip` (price board, how-it-works, station cards, counter cards — HTML + PDF), `session-script.pdf` (`data/host-script.md` exported to PDF — Google Docs → File → Download → PDF).
